...
**문제**: 한국 시간 설정을 했지만 윈도우에서 실행 시 시간이 제대로 표시되지 않음
**해결**:
- **Docker 컨테이너 환경으로 플랫폼 독립성 확보**: 호스트 OS와 무관하게 일관된 Linux 환경 제공
- **명시적 타임존 설정**: Dockerfile에서 Asia/Seoul 타임존 강제 설정
- **시간 계산 로직 개선**: 플랫폼 호환 방식으로 시간 처리 로직 수정
- **볼륨 마운트**: 호스트의 시간 설정을 컨테이너와 동기화
